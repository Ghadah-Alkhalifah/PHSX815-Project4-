...
X=5 #Size of the lattice
Y=5 # Size of the lattice
# Set up the lattice
seed=5555
random.seed(seed)
...
def quantity(X, Y, T_min, T_max, T_step, MCsweep, Nbootstrap):
    ...
    def Pfun(x, n, m, l):
        ...
        value=z**(-l)*n 
        return value
    # Fit the power function to the specific heat data
    p0 = [1, 2.269, 0.5]  # Initial guess for the parameter values
    popt, pcov = curve_fit(Pfun, temperatures, S_heat, p0=p0)
    ...
    for i in range(Nbootstrap):
        ...
        # Fit the power function to the bootstrap sample
        popt, pcov = curve_fit(Pfun, temperatures, bootstrap_specific_heats, p0=p0)
...
MC_simulation = quantity(X=5, Y=5, T_min=1.3, T_max=3.3, T_step=0.007, MCsweep = 10000, Nbootstrap=1000) 
